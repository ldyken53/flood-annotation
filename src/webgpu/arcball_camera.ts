 ...
 * coordinates.
 */
import {vec2, vec3, vec4, quat, mat4, mat3} from 'gl-matrix';
export class ArcballCamera {
  ...
  rotate(prevMouse, curMouse) {
    var mPrev = vec2.set(
      vec2.create(),
      this.clamp(prevMouse[0] * 2.0 * this.invScreen[0] - 1.0, -1.0, 1.0),
      this.clamp(1.0 - prevMouse[1] * 2.0 * this.invScreen[1], -1.0, 1.0)
    );

    var mCur = vec2.set(
      vec2.create(),
      this.clamp(curMouse[0] * 2.0 * this.invScreen[0] - 1.0, -1.0, 1.0),
      this.clamp(1.0 - curMouse[1] * 2.0 * this.invScreen[1], -1.0, 1.0)
    );
  ...
  pan(mouseDelta) {
    ...
    var translation = mat4.fromTranslation(mat4.create(), worldDelta as Float32Array);
